...
if __name__ == '__main__':
    ...
    class TestLinearRegressionFunctions(unittest.TestCase):
        def test_linear_regression(self):
            ...
            self.assertAlmostEqual(b, 2.2, places=2)

        def test_predict_stock_value(self):
            stocks_data = {
                1680969060: 5,
                1680969360: 7,
                1680969660: 546,
            ...
            epoch_moment = 1680970000
            predicted_value = predict_stock_value(stocks_data, epoch_moment)
            self.assertTrue(isinstance(predicted_value, float))

        def test_stocks_bought_weight(self):
            stocks = 55
            weight = stocks_bought_weight(stocks)
            expected_weight = 1 / ((5 + 55 - 50) / 50)
            self.assertAlmostEqual(weight, expected_weight, places=2)

        def test_adjusted_slope_price(self):
            data_example = {
                1680969060: 5,
                1680969360: 7
            }
            stocks = 55
            adj_slope = adjusted_slope_price(data_example, stocks)
            self.assertTrue(isinstance(adj_slope, float))

        def test_expected_stock_price_with_adjustment(self):
            data_example = {
                1680969060: 5,
                1680969360: 7
            }
            ...
            expected_value = expected_stock_price_with_adjustment(data_example,
                                                                  adj_slope,
                                                                  current)
            self.assertTrue(isinstance(expected_value, float))
